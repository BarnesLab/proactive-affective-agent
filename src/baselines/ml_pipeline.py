...
Runs all models x all targets x 5-fold CV using the same splits as LLM versions.
No LLM calls needed -- can run anytime without worrying about API limits.

Supports parallelism at the (model, target) level within each fold via joblib,
which can dramatically speed up execution on multi-core servers (e.g. 40-core).
"""
...
# ---------------------------------------------------------------------------
# Standalone functions for parallel execution (must be top-level for pickling)
# ---------------------------------------------------------------------------

def _run_single_regression_job(
    model_name: str,
    target: str,
    ...
    """Fit one regression model on one target for one fold. Returns result dict or None."""
    try:
        model = MLBaseline(model_name, task="regression", n_jobs=n_jobs_inner)
...
def _run_single_classification_job(
    model_name: str,
    target: str,
    X_train_np: np.ndarray,
    y_tr_int: np.ndarray,
    ...
    """Fit one classification model on one target for one fold. Returns result dict or None."""
    try:
        model = MLBaseline(
            model_name, task="classification", n_jobs=n_jobs_inner,
...
class MLBaselinePipeline:
    """Run all models x all targets x 5-fold CV using existing splits.

    When ``parallel=True`` (default), all (model, target) pairs within each
    fold are dispatched via ``joblib.Parallel`` for maximum CPU utilization.
    ...
    def __init__(
        ...
        Args:
            ...
            n_jobs: Parallelism for GridSearchCV and RF (default: -1 = all cores).
                When parallel=True, this controls the outer parallelism level;
                inner GridSearchCV will use n_jobs=1.
            parallel: If True (default), parallelize (model, target) pairs within
                each fold using joblib. Set to False to revert to sequential mode.
        """
    ...
    def _build_fold_jobs(
        ...
        X_train_np: np.ndarray,
        X_test_np: np.ndarray,
        y_cont_train: pd.DataFrame,
        y_cont_test: pd.DataFrame,
        y_bin_train: pd.DataFrame,
        y_bin_test: pd.DataFrame,
    ) -> list[tuple]:
        """Build a list of (delayed_callable, ...) jobs for one fold.

        Returns a list of tuples:
            ("regression", model_name, target, delayed_fn_args...)
            ("classification", model_name, target, delayed_fn_args...)
        Each entry contains everything needed to dispatch one parallel job.
        """
        from src.utils.mappings import BINARY_STATE_TARGETS, CONTINUOUS_TARGETS

        # Determine inner n_jobs: 1 in parallel mode to avoid oversubscription
        n_jobs_inner = 1 if self.parallel else self.n_jobs
        ...
        for target in CONTINUOUS_TARGETS:
            ...
            for model_name in reg_models:
                jobs.append(
                    delayed(_run_single_regression_job)(
                        model_name=model_name,
                        target=target,
        ...
        for target in binary_targets:
            ...
            for model_name in clf_models:
                jobs.append(
                    delayed(_run_single_classification_job)(
                        model_name=model_name,
                        target=target,
                        X_train_np=X_train_np,
                        y_tr_int=y_tr_int,
        ...
        for fold in (folds if folds is not None else range(1, 6)):
            ...
            if self.parallel:
                self._run_fold_parallel(
                    fold, X_train_np, X_test_np,
                    y_cont_train, y_cont_test,
                    y_bin_train, y_bin_test,
                    all_results,
                )
            else:
                self._run_fold_sequential(
                    fold, X_train_np, X_test_np,
                    y_cont_train, y_cont_test,
                    y_bin_train, y_bin_test,
                    all_results,
    ...
    def _run_fold_parallel(
        ...
        """Run all (model, target) jobs for one fold in parallel via joblib."""
        jobs = self._build_fold_jobs(
            fold, X_train_np, X_test_np,
            y_cont_train, y_cont_test,
            y_bin_train, y_bin_test,
        )

        n_jobs_outer = self.n_jobs  # -1 means all cores
        logger.info(
            f"  Fold {fold}: dispatching {len(jobs)} (model, target) jobs "
            f"with n_jobs={n_jobs_outer} (inner GridSearchCV n_jobs=1)"
        )

        results = Parallel(n_jobs=n_jobs_outer, prefer="processes")(jobs)

        # Aggregate results into all_results dict
        for result in results:
            ...
            if result["task_type"] == "regression":
                ...
                all_results.setdefault(model_name, {}).setdefault(target, []).append({
                    ...
                    "n_train": result["n_train"],
                    "n_test": result["n_test"],
                    "prevalence_train": result["prevalence_train"],
                    "prevalence_test": result["prevalence_test"],
                })
    ...
    def _run_fold_sequential(
        ...
        """Run all (model, target) jobs for one fold sequentially (original behavior)."""
        from src.utils.mappings import BINARY_STATE_TARGETS, CONTINUOUS_TARGETS
    ...
    def _aggregate_folds(self, all_results: dict) -> dict[str, Any]:
        """Average metrics across 5 folds for each model x target."""
        aggregated = {}
