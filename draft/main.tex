...
To understand how agentic agents leverage their sensing tools, we analyze the tool call patterns across all autonomously-agentic predictions.

\todo{Complete tool usage analysis: (1) mean/median tool calls per prediction by variant, (2) per-tool call frequency distribution, (3) tool call sequences and common investigation patterns, (4) correlation between number/type of tool calls and prediction accuracy.}
...
ML baselines use 5-fold CV on the full dataset ($\sim$15,984 entries across 418 users), while LLM agents are evaluated on a 50-user subset (${\sim}$3,900 entries per variant). Although the evaluation populations differ, the 50-user sample spans all 5 cross-validation folds and provides sufficient statistical power for the consistent patterns we observe. Furthermore, the LLM agents are evaluated on a \emph{subset} of the same participants used by ML baselines, so the comparison is conservative: any distributional advantage would favor the baselines, which train and evaluate on the full population.
...
Our work provides evidence that LLM-based agentic sensing is a promising paradigm for proactive JITAI systems. By combining the contextual reasoning capabilities of large language models with the passive, always-on nature of smartphone sensing, we move closer to systems that can support cancer survivors precisely when they need it most---even when they are not actively seeking help. While \sysshort{} demonstrates proactive \emph{prediction} of intervention opportunities, the translation from prediction to effective intervention delivery remains an important direction for future work requiring prospective evaluation.
