...
%% Abstract
\begin{abstract}
Proactive mental health support requires detecting intervention opportunities from behavior alone---yet the people who most need help are least likely to self-report. \sysshort{} reframes this challenge by deploying LLM agents that \emph{autonomously investigate} raw smartphone sensor streams (motion, GPS, screen, keyboard) through tool-use, reasoning about behavioral patterns the way a clinician would---without pre-formatted feature summaries. We evaluate a 2$\times$2 factorial design (structured vs.\ agentic reasoning $\times$ sensing-only vs.\ multimodal input) on 18 cancer survivors from the BUCS longitudinal study. Agentic agents consistently outperform structured LLM pipelines and traditional ML baselines, achieving balanced accuracy of 0.749 for \erdesire{} detection, 0.760 for negative affect, and mean BA of 0.669 across all targets---surpassing the best non-LLM baseline (BA\,=\,0.629). The critical finding: letting an LLM decide \emph{what} to query outperforms giving it everything upfront, establishing agentic sensing as a viable paradigm for just-in-time adaptive interventions.
\end{abstract}
