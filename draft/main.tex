...
Results show that agentic multimodal agents achieve the best performance across key targets: balanced accuracy of 0.749 for emotion regulation desire detection (V4/V6), 0.733--0.738 for intervention availability (V2/V4/V6), and 0.760 for negative affect state detection (V6), substantially outperforming both structured LLM pipelines and traditional ML baselines. Overall, V6 achieves mean BA = 0.669 and V4 achieves mean BA = 0.666, substantially outperforming the best non-LLM baseline (MiniLM text embeddings, BA = 0.629). The agentic approach---where the LLM autonomously decides which sensor data to query---consistently outperforms structured pipelines that receive pre-formatted feature summaries, demonstrating the value of autonomous behavioral investigation. Cross-user retrieval-augmented generation provides effective calibration anchors from population-level data. Our findings provide preliminary evidence that LLM-based agentic sensing is a promising paradigm for proactive mental health support in cancer survivorship.
\end{abstract}
