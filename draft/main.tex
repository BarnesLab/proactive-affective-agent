...
\begin{enumerate}[nosep]
    ...
    \item \textbf{LLM agents substantially outperform traditional ML on binary state detection}. The best LLM agents (V6, BA=0.669; V4, BA=0.666) outperform the best ML baseline (MiniLM text embeddings, BA=0.629).
...
The Model Context Protocol (MCP)~\cite{anthropic2024mcp} provides a standardized interface for connecting LLMs to external data sources and tools, enabling modular tool integration.

\paragraph{Harness Engineering.}
A growing body of practice recognizes that the performance of agentic systems depends critically on how tools, prompts, and constraints are designed around the base model---what practitioners term \emph{harness engineering}~\cite{anthropic2025claude_code}. While the underlying LLM provides general reasoning, the harness---tool definitions, system prompts, output schemas, retrieval pipelines, and guardrails---shapes \emph{what the agent can perceive and do}. In our system, harness engineering is the primary design activity: we define five purpose-built sensing tools (targeted sensor queries, daily behavioral summaries, personal baseline comparison, receptivity history retrieval, and similar-day matching), each with carefully scoped parameters that expose the right granularity of behavioral data without overwhelming the agent's context. The tool interface design determines which behavioral signals the agent can access, at what temporal resolution, and with what contextual framing---decisions that directly impact prediction quality. We further design structured output schemas that constrain predictions to valid ranges and enforce coverage of all target variables. This harness-centric view---where the ``intelligence'' of the system emerges from the interplay between a general-purpose reasoner and domain-specific tooling---contrasts with approaches that rely solely on prompt engineering or fine-tuning to embed domain knowledge into the model itself.
