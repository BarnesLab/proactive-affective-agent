...
\begin{enumerate}[nosep]
    \item \textbf{Agentic investigation outperforms structured pipelines} for binary state detection. The autonomously-agentic multimodal agents (Auto-Multi/Auto-Multi+) achieve balanced accuracy (BA) of 0.751 for \erdesire{} state detection vs.\ 0.664 for the structured-agentic multimodal agent (Struct-Multi), and Auto-Multi+ achieves 0.722 for negative affect state detection vs.\ 0.674 for Struct-Multi.

    \item \textbf{Multimodal data (sensing + diary) outperforms either modality alone}. Multimodal agents (Struct-Multi, Auto-Multi, Auto-Multi+) consistently outperform both sensing-only (Struct-Sense, Auto-Sense) and diary-only (CALLM) approaches, with Auto-Multi+ achieving mean BA of 0.661 across all binary targets.

    \item \textbf{LLM agents substantially outperform traditional ML on binary state detection}. The best LLM agents (Auto-Multi+, BA=0.661; Auto-Multi, BA=0.660) outperform the best ML baseline (MiniLM text embeddings, BA=0.629).
...
We evaluate all agent variants on 50 cancer survivors drawn from the BUCS test sets. These participants span 5 cross-validation folds, yielding approximately 3,900 EMA entries per agent variant (3,851--3,955 depending on version-specific parsing). Each participant contributes 70--96 chronologically ordered EMA entries, evaluated sequentially to simulate real-time deployment.
...
\begin{table}[t]
\caption{Aggregate performance across all prediction targets (50 users, ${\sim}$3,900 entries per variant). Best in each column is \textbf{bold}. $\dagger$ML baselines evaluated via 5-fold CV on full dataset (15,984 entries).}
...
Auto-Multi & Multimodal & 0.660 & 0.657 \\
Auto-Multi+ & Multimodal (filtered) & \textbf{0.661} & \textbf{0.656} \\
\addlinespace
...
The agentic advantage is not driven by outlier participants. With all 50 participants evaluated across all agent variants, Auto-Multi outperforms Struct-Sense for the vast majority of participants on per-user mean BA across all binary targets. Similarly, Auto-Multi+ outperforms Struct-Multi for most participants. The consistency of the agentic advantage across 50 participants with diverse base rates and sensing coverage patterns suggests the improvement is genuine rather than a sampling artifact.
...
The sensing-only agents (Auto-Sense: BA = 0.589, Auto-Sense+: BA = 0.591) achieve performance above chance and above all traditional ML baselines---demonstrating that proactive prediction from passive sensing alone is viable, if imperfect. In practice, a deployed system could use multimodal prediction when diary entries are available and fall back to sensing-only prediction when they are not, combining reactive accuracy with proactive coverage.
...
Our evaluation covers 50 participants (${\sim}$3,900 entries per variant) with all 7 agent versions fully complete. While results are consistent across participants and the patterns are clear, the evaluation subset (50 of 418 total participants) reflects the computational cost of LLM inference: each agentic prediction requires 30--90 seconds and multiple tool calls, making full-dataset evaluation (15,984 entries $\times$ 7 variants) prohibitive.
...
ML baselines use 5-fold CV on the full dataset ($\sim$15,984 entries across 418 users), while LLM agents are evaluated on a 50-user subset (${\sim}$3,900 entries). Direct comparison requires caution, though the consistent pattern---LLM agents outperform ML on binary state detection---is robust to this difference, and the 50-user sample provides substantially more statistical power than smaller pilot studies.
