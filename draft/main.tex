...
We evaluate \sysshort{} through a 2$\times$2 factorial design comparing \emph{structured} vs.\ \emph{agentic} reasoning and \emph{sensing-only} vs.\ \emph{multimodal} (sensing + diary) data, plus a diary-only baseline, across 18 cancer survivors from the BUCS longitudinal study (1,567 EMA entries per condition for the four fully-complete versions). Our primary prediction targets are \emph{intervention opportunity}---the conjunction of emotion regulation desire and intervention availability---and positive/negative affect states.

Results show that agentic multimodal agents achieve the best performance across key targets: balanced accuracy of 0.749 for emotion regulation desire detection (V4/V6), 0.733--0.738 for intervention availability (V2/V4/V6), and 0.760 for negative affect state detection (V6), substantially outperforming both structured LLM pipelines and traditional ML baselines. Overall, V6 achieves mean BA = 0.669 and V4 achieves mean BA = 0.666, both beating the autocorrelation baseline (0.658). The agentic approach---where the LLM autonomously decides which sensor data to query---consistently outperforms structured pipelines that receive pre-formatted feature summaries, demonstrating the value of autonomous behavioral investigation. Cross-user retrieval-augmented generation provides effective calibration anchors from population-level data. Our findings provide preliminary evidence that LLM-based agentic sensing is a promising paradigm for proactive mental health support in cancer survivorship.
\end{abstract}
...
\begin{enumerate}[nosep]
    \item \textbf{Agentic investigation outperforms structured pipelines} for binary state detection. The agentic multimodal agents (V4/V6) achieve balanced accuracy (BA) of 0.749 for \erdesire{} state detection vs.\ 0.734 for the structured multimodal agent (V3), and V6 achieves 0.760 for negative affect state detection vs.\ 0.727 for V3.

    \item \textbf{Multimodal data (sensing + diary) outperforms either modality alone}. Multimodal agents (V3/V4/V6) consistently outperform both sensing-only (V1/V2) and diary-only (CALLM) approaches, with V6 achieving mean BA of 0.669 across all binary targets.

    \item \textbf{LLM agents substantially outperform traditional ML on binary state detection}. The best LLM agents (V6, BA=0.669; V4, BA=0.666) outperform the best ML baseline (MiniLM text embeddings, BA=0.629) and the AR baseline (BA=0.658).
...
V6 additionally receives a pre-computed daily behavioral narrative---a filtered, structured summary of the day's sensing data with redundant and zero-information periods removed---reducing the raw data volume by 7.4$\times$ while preserving salient signals. We also evaluate V5, a sensing-only filtered variant (V2 + filtered narrative); V5 and V6 test whether data quality filtering improves prediction beyond the base agentic approach.
...
We evaluate all agent variants on a pilot subset of 18 cancer survivors drawn from the BUCS test sets. These participants span 5 cross-validation folds, yielding 1,567 EMA entries per fully-complete agent variant. \todo{Update participant list for N=18. CALLM: 13/18 users, results will update. V1: 13/18 users. V3: only 10/18 users, bottleneck. V2/V4/V5/V6: 18/18 complete.} Each participant contributes 81--93 chronologically ordered EMA entries, evaluated sequentially to simulate real-time deployment.
...
Table~\ref{tab:main_results} and Figure~\ref{fig:ba_comparison} present aggregate results across all prediction targets. The agentic multimodal variants (V4, V6) achieve the highest overall balanced accuracy and F1, while the diary-based approaches (CALLM, V3) achieve lower continuous MAE.
...
CALLM & Diary + RAG & 3.661 & 0.624 & 0.611 \\ \todo{CALLM: 13/18 users, results will update}
V1 (Structured) & Sensing & 5.502 & 0.523 & 0.465 \\ \todo{V1: 13/18 users}
V2 (Agentic) & Sensing & 4.881 & 0.601 & 0.594 \\
V3 (Structured) & Multimodal & \textbf{3.947} & 0.638 & 0.632 \\ \todo{V3: only 10/18 users, bottleneck}
V4 (Agentic) & Multimodal & 4.353 & 0.666 & 0.660 \\
V6 (Agentic+Filtered) & Multimodal & 4.220 & \textbf{0.669} & \textbf{0.663} \\
\addlinespace
...
\midrule
CALLM & 0.678 & 0.697 & 0.531 & 0.521 & 1.572 \\ \todo{CALLM: 13/18 users, results will update}
V1 & 0.533 & 0.518 & 0.494 & 0.396 & 2.802 \\ \todo{V1: 13/18 users}
V2 & 0.669 & 0.672 & 0.733 & 0.732 & 1.316 \\
V3 & 0.734 & 0.751 & 0.565 & 0.561 & 1.740 \\ \todo{V3: only 10/18 users, bottleneck}
V4 & \textbf{0.749} & \textbf{0.753} & \textbf{0.738} & \textbf{0.737} & 1.010 \\
V6 & \textbf{0.749} & 0.748 & 0.735 & 0.734 & \textbf{0.978} \\
\midrule
AR & 0.658 & --- & --- & --- & 1.646 \\
...
Detecting when a cancer survivor \emph{wants} emotional support is the most direct signal for intervention timing. The agentic multimodal agents achieve strong performance: V4 and V6 both attain BA = 0.749, with V6 achieving the lowest MAE = 0.978 (on a 0--10 scale), outperforming both the structured multimodal agent (V3: BA = 0.734) and CALLM (BA = 0.678). The agentic sensing-only agent (V2: BA = 0.669) also outperforms V1 (BA = 0.533), demonstrating the value of autonomous investigation even without diary text.

\paragraph{Intervention Availability.}
Predicting whether a survivor is \emph{available} to engage with an intervention shows a striking pattern: agentic agents dramatically outperform structured ones. V4 (BA = 0.738) and V6 (BA = 0.735) far exceed V3 (BA = 0.565) and CALLM (BA = 0.531). Even the sensing-only agentic agent V2 (BA = 0.733) substantially outperforms all structured variants. This suggests that \emph{behavioral signals}---detectable through autonomous sensing investigation---are more informative for availability prediction than diary text alone. A user's diary may describe their emotions but rarely states whether they are currently available for an intervention.

\paragraph{Intervention Opportunity (Joint Prediction).}
For a JITAI to act, both desire and availability must be predicted correctly. We evaluate the joint prediction where both \erdesire{} State = True \emph{and} \intavail{} = yes. The ground-truth joint rate is 19.2\% (184/956 entries). \todo{Update joint rate for N=18 / 1,567 entries.} The agentic multimodal agents achieve the best joint detection: V4 achieves joint precision = 0.610, recall = 0.620, F1 = 0.615, substantially outperforming CALLM (F1 = 0.438) and V1 (F1 = 0.148). V6 performs similarly (F1 = 0.603). This confirms that agentic multimodal agents excel not just on individual constructs but on the clinically actionable conjunction. In contrast, CALLM performs well on desire (0.678) but poorly on availability (0.531), causing it to miss the joint opportunity.
...
\midrule
CALLM & 0.566 & 0.545 & 0.681 & 0.700 & 5.147 & \textbf{4.264} \\ \todo{CALLM: 13/18 users, results will update}
V1 & 0.522 & 0.406 & 0.520 & 0.510 & 7.033 & 6.671 \\ \todo{V1: 13/18 users}
V2 & 0.604 & 0.603 & 0.633 & 0.609 & 6.023 & 7.304 \\
V3 & 0.601 & 0.580 & 0.727 & 0.740 & 5.899 & 4.201 \\ \todo{V3: only 10/18 users, bottleneck}
V4 & 0.738 & 0.734 & 0.753 & 0.725 & 5.436 & 6.615 \\
V6 & \textbf{0.755} & \textbf{0.750} & \textbf{0.760} & \textbf{0.738} & \textbf{5.385} & 6.298 \\
\midrule
...
Detecting elevated negative affect---the strongest clinical signal for intervention need---is where agentic multimodal agents shine. V6 achieves BA = 0.760, above V3 (0.727) and CALLM (0.681). This represents a meaningful improvement: at 76\% balanced accuracy, the system correctly identifies roughly 3 out of 4 instances of elevated negative affect, while maintaining a low false positive rate.

\paragraph{Positive Affect State Detection.}
Detecting unusual positive affect states follows the same pattern. V6 (BA = 0.755) substantially outperforms V3 (0.601) and CALLM (0.566). Positive affect tracking is valuable for identifying moments of resilience and wellbeing that can be reinforced through positive psychology interventions.
...
The agentic advantage is not driven by outlier participants. Examining per-user mean BA across all binary targets, V4 outperforms V1 for the majority of participants. \todo{Update per-user consistency stats for N=18. V1 only 13/18 users.} Similarly, V6 outperforms V3 for most participants. \todo{V3 only 10/18 users; per-user comparison limited.} The consistency of the agentic advantage across participants with diverse base rates and sensing coverage patterns suggests the improvement is genuine rather than a sampling artifact.
...
\begin{table*}[t]
\caption{Balanced accuracy for all binary targets across LLM agent variants (18 users, 1,567 entries for fully-complete versions). Bold = best per row. Targets above the line are focus targets discussed in the main text; those below are supplementary. \todo{CALLM: 13/18, V1: 13/18, V3: 10/18 users---these values are interim.}}
\label{tab:full_ba}
\small
