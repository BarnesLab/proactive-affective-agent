...
Results show that agentic multimodal agents achieve the best performance across key targets: balanced accuracy of 0.786 for emotion regulation desire detection, 0.743 for intervention availability, and 0.790 for negative affect state detection, substantially outperforming both structured LLM pipelines and traditional ML baselines. The agentic approach---where the LLM autonomously decides which sensor data to query---consistently outperforms structured pipelines that receive pre-formatted feature summaries, demonstrating the value of autonomous behavioral investigation. Cross-user retrieval-augmented generation provides effective calibration anchors from population-level data. Our findings provide preliminary evidence that LLM-based agentic sensing is a promising paradigm for proactive mental health support in cancer survivorship.
\end{abstract}
